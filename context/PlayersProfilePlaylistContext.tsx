...
interface PlayersProfiePlaylistContextType {
  playlists: Playlist[];
...
const PlayersProfiePlaylistContext = createContext<
  PlayersProfiePlaylistContextType | undefined
>(undefined);

/*  HOOK  */

export const usePlayersProfiePlaylist = () => {
  const context = useContext(PlayersProfiePlaylistContext);

  if (!context) {
    throw new Error(
      "usePlayersProfiePlaylist must be used inside PlayersProfiePlaylistProvider"
    );
  }
...
export const PlayersProfiePlaylistProvider = ({
  children,
}: {
  children: ReactNode;
}) => {
  ...
  const fetchPlaylists = async () => {
    try {
      ...
      const res = await axios.get(
        `${API_BASE_URL}/api/playersprofile-playlist`
      );
  ...
  const fetchSinglePlaylist = async (id: string) => {
    try {
      ...
      const res = await axios.get(
        `${API_BASE_URL}/api/playersprofile-playlist/${id}`
      );
  ...
  const createPlaylist = async (
    ...
    try {
      ...
      const transformedPayload = {
        team360PostId: payload.playerProfilesId,
  ...
  return (
    <PlayersProfiePlaylistContext.Provider
      value={{
        playlists,
        singlePlaylist,
        loading,
