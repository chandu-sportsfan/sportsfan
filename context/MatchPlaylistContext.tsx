...
interface MatchPlaylistContextType {
  playlists: Playlist[];
...
const MatchPlaylistContext = createContext<
  MatchPlaylistContextType | undefined
>(undefined);

/*  HOOK  */

export const useMatchPlaylist = () => {
  const context = useContext(MatchPlaylistContext);

  if (!context) {
    throw new Error(
      "useMatchPlaylist must be used inside MatchPlaylistProvider"
    );
  }
...
export const MatchPlaylistProvider = ({
  children,
}: {
  children: ReactNode;
}) => {
  ...
  const fetchPlaylists = async () => {
    try {
      ...
      const res = await axios.get(
        `${API_BASE_URL}/api/matchesplaylist`
      );
  ...
  const fetchSinglePlaylist = async (id: string) => {
    try {
      ...
      const res = await axios.get(
        `${API_BASE_URL}/api/matchesplaylist/${id}`
      );
