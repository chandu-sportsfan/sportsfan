...
function toCountryWords(country: string): string[] {
  return country
    .toLowerCase()
    .replace(/[^a-z0-9\s]/g, "")
    .split(" ")
...
async function main() {
  console.log("Fetching all wt20Clubs documents...");
  const snapshot = await db.collection("wt20Clubs").get();
  ...
  for (let i = 0; i < snapshot.docs.length; i += BATCH_SIZE) {
    ...
    for (const doc of chunk) {
      ...
      const words    = toCountryWords(country);

      const alreadyDone =
        data.country_lower === expected &&
        Array.isArray(data.country_words) &&
        data.country_words.length === words.length;
      ...
      batch.update(doc.ref, {
        country_lower: expected,
        country_words: words,   // e.g. ["new", "zealand"]
      });
      updated++;
    }
