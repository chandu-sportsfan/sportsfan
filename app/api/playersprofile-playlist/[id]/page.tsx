...
export async function PUT(req: NextRequest) {
  try {
    ...
    let playerProfilesId: string = existingData?.playerProfilesId || "";
    ...
    if (isFormData) {
      ...
      for (let i = 0; i < audioFiles.length; i++) {
        ...
        const uploadRes = await cloudinary.uploader.upload(base64, {
          folder: `playerProfiles/playlists/${playerProfilesId}/audio`,
          resource_type: 'video',
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    await db.collection("playerProfilePlaylists").doc(id).delete();
