...
export async function PUT(req: NextRequest) {
  try {
    ...
    const existing = await db.collection("playerInsights").doc(id).get();
    if (!existing.exists) {
      return NextResponse.json(
        { success: false, message: "Insights doc not found" },
        { status: 404 }
    ...
    await db.collection("playerInsights").doc(id).update(updateData);
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    await db.collection("playerInsights").doc(id).delete();
