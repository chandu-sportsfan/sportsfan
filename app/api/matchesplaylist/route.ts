  ...
  try {
    ...
    if (!matchesPlaylistId) {
      return NextResponse.json(
        { success: false, message: "matchesPlaylistIdId is required" },
        { status: 400 }
      );
    }
    ...
    for (let i = 0; i < videoFiles.length; i++) {
      ...
      if (videoThumbnails[i]) {
        ...
        const thumbUpload = await cloudinary.uploader.upload(thumbBase64, {
          folder: `macthes/playlists/${matchesPlaylistId}/video/thumbnails`,
    ...
    const docRef = await db.collection("matchPlaylists").add(playlistData);
...
export async function GET(req: NextRequest) {
  try {
    ...
    const collectionRef = db.collection("matchPlaylists");
    let query: FirebaseFirestore.Query = collectionRef;

    // Apply filter if matchesPlaylistId is provided
    if (matchesPlaylistId) {
    ...
    if (lastDocId && lastDocCreatedAt) {
      const lastDocRef = db.collection("matchesPlaylistId").doc(lastDocId);
