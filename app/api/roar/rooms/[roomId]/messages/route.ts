...
// ── Message types that hold MULTIPLE independent trivia questions ────────────
const MULTI_QUESTION_ANSWERABLE_TYPES = new Set(["trivia"]);
  ...
  try {
    ...
    if (since) {
      if (channelId || channelSlug) {
        query = query.where("createdAt", ">", parseInt(since, 10));
      } else {
        query = messagesRef
          .where("createdAt", ">", parseInt(since, 10))
          .orderBy("createdAt", "desc")
    ...
    // Build per-question answer lookup map (trivia)
    const userTriviaAnswersByIndex = new Map<
      number,
      Record<number, { selectedOption: string; isCorrect: boolean }>
    >();
