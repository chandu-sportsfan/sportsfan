...
type PredictionVote = { vote?: "agree" | "disagree" };
...
export async function POST(
  ...
  try {
    ...
    const { correctVote }: { correctVote?: "agree" | "disagree" } = await req.json();
    if (correctVote !== "agree" && correctVote !== "disagree") {
      return NextResponse.json({ error: "correctVote must be 'agree' or 'disagree'" }, { status: 400 });
    }
