...
export default function CricketMatchesListPage() {
  ...
  const fetchMatches = useCallback(async () => {
    ...
    try {
      ...
      if (res.data.success) {
        const apiMatches = res.data.matches as Match[];
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="flex gap-4 mb-6">
        ...
        <button onClick={() => { setFilterSeason(""); setFilterTeam(""); }} className="px-3 py-2 bg-gray-700 rounded">Clear</button>
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {matches.map((match) => (
            <Link href={`/admin/cricket-matches-management/cricketmatches-list/${match.id}`} key={match.id}>
