...
export default function AudioMessagesListPage() {
  ...
  const fetchMessages = useCallback(async (currentFilter: "all" | "unread" | "flagged") => {
    try {
      ...
      let url = "/api/audio-messages";
      if (currentFilter === "unread") url = "/api/audio-messages?status=unread";
      else if (currentFilter === "flagged") url = "/api/audio-messages?status=flagged";
  ...
  const handleMarkAsRead = async (messageId: string, currentIsRead: boolean) => {
    ...
    try {
      const response = await axios.patch("/api/audio-messages", {
        messageId,
  ...
  const handleDeleteMessage = async (messageId: string) => {
    ...
    try {
      await axios.delete(`/api/audio-messages?messageId=${messageId}`);
  ...
  return (
    <div className="max-w-[1440px] mx-auto p-6 text-white">
      {/* HEADER */}
      <div className="mb-6">
        <h1 className="text-xl font-semibold">Audio Messages</h1>
        <p className="text-sm text-gray-400">
          Manage user messages and feedback for audio drops
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-[#161b22] border border-[#21262d] rounded-lg p-4">
          <p className="text-sm text-gray-400">Unique Audios</p>
          <p className="text-2xl font-bold text-blue-400">{stats.totalAudios}</p>
        </div>
      </div>
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <div className="max-w-[200px]">
                        <p className="text-sm text-white truncate" title={message.audioTitle}>
                          {message.audioTitle}
                        </p>
                      </div>
                    </td>
        ...
        >
          ...
          >
            ...
            <div className="space-y-4">
              ...
              <div>
                <p className="text-xs text-gray-400 uppercase">Audio</p>
                <p className="text-white">{selectedMessage.audioTitle}</p>
                <p className="text-xs text-gray-500">ID: {selectedMessage.audioId}</p>
              </div>
