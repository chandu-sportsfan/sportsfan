...
export default function AddBattleArenaForm() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setFormError(e.response?.data?.error || e.message || "An error occurred");
    } finally {
      setSubmitting(false);
    }
  };
                ...
                <div className="flex p-1 bg-white/5 border border-white/10 rounded-xl w-fit">
                  <button
                    ...
                    className={`flex items-center gap-2 px-6 py-2.5 rounded-lg text-sm font-semibold transition-all ${
                      battleType === "PLAYERS" ? "bg-purple-600 text-white shadow-lg" : "text-gray-400 hover:text-white"
                    }`}
                  >
                    <User size={16} />
                    Players
                  ...
                  <button
                    ...
                    className={`flex items-center gap-2 px-6 py-2.5 rounded-lg text-sm font-semibold transition-all ${
                      battleType === "CLUBS" ? "bg-purple-600 text-white shadow-lg" : "text-gray-400 hover:text-white"
                    }`}
                  >
                    <Shield size={16} />
                    Clubs
